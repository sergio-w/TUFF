    ...
    function createNewWindow() {
        ...
        const editor = ace.edit("pAdeblc-txeditor");
    ...
    function syntaxhighlight(id) {
        const codeEditor = document.getElementById('codeEditor');
        const keywords = ['let', 'if', 'else', 'console', 'log'];
        
        codeEditor.addEventListener('input', function () {
            let code = codeEditor.innerText; // Get the text content
            let tokens = code.split(/(\s+|\b)/); // Split the text into tokens (spaces and words)
            
            // Apply syntax highlighting
            tokens = tokens.map(function (token) {
                if (keywords.includes(token)) {
                    return `<span class="keyword">${token}</span>`; // Highlight keywords
                } else if (/[\+\-\*\/=]/.test(token)) {
                    return `<span class="operator">${token}</span>`; // Highlight operators
                }
                return token; // Return other tokens as they are
            });
        
            // Set the innerHTML to highlight the tokens
            codeEditor.innerHTML = tokens.join(''); // Rebuild the HTML with highlighted tokens
        });
